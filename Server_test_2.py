...
class Server(threading.Thread):
    def __init__(self):
        super().__init__()
        self.rcv_msg = None
        self.TCP_IP = '172.162.1.5'  # Replace with your server's IP address
        self.TCP_PORT = 2203  # Choose the same port number as used in the client script
    ...
    def run(self):
        try:
            ...
            while True:
                self.rcv_msg = conn.recv(self.BUFFER_SIZE).decode('utf-8')  # Receive data from the client
                if not self.rcv_msg:
                    break
